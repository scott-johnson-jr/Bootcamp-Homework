...
fun describe(result: TransferResult): String =
    when (result) {
        is TransferResult.Success -> "Transfer Completed: ${result.confirmationId}"
...
suspend fun fetchAccounts(): List<Account> {
    ...
    delay(500)
    return listOf(
        Account("101", "Checking", -11.50),
        Account("102", "Savings", 40.50),
        Account("103", "Reserve", 105.45)
    )
}
