...
let greeting: String = "Welcome to \(appName), \(fullName). Your account is active."
...
//     // if amount is less than or equal to zero.
    ...
    guard amount <= availableBalance else {
        return "Insufficient funds. Available: \(String(format: "%.2f", averageTransaction))"
    }
    return "Approved"
}

// Test your function:
print(processWithdrawal(amount: -50, availableBalance: 1000))       // Invalid amount -------- UN COMMENT
print(processWithdrawal(amount: 2000, availableBalance: 1000))      // Insufficient funds -------- UN COMMENT
print(processWithdrawal(amount: 500, availableBalance: 1000))       // Approved -------- UN COMMENT
...
// "• Savings"
// etc.
let accountTypes = ["Checking", "Savings", "Invesment", "Credit Card"]
for accounts in accountTypes {
    print(accounts)
}
...
for (index, accountTypes) in accountTypes.enumerated() {
    print("\(accountTypes) occurs at index \(index)")
}
